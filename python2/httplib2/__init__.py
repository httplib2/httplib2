...
__all__ = [
    ...
    'UnimplementedHmacDigestAuthOptionError', 'debuglevel',
...
def _get_end2end_headers(response):
  ...
  hopbyhop.extend(
      [x.strip() for x in response.get('connection', '').split(',')])
...
def _entry_disposition(response_headers, request_headers):
  ...
  if 'pragma' in request_headers and request_headers['pragma'].lower().find(
      'no-cache') != -1:
    retval = 'TRANSPARENT'
...
def _cnonce():
  dig = _md5('%s:%s' % (time.ctime(), [
      '0123456789' [random.randrange(0, 9)] for i in range(20)
  ])).hexdigest()
  return dig[:16]
...
def _wsse_username_token(cnonce, iso_now, password):
  return base64.b64encode(
      _sha('%s%s%s' % (cnonce, iso_now, password)).digest()).strip()
...
class DigestAuthentication(Authentication):
  ...
  def __init__(self, credentials, host, request_uri, headers, response, content,
    ...
    self.A1 = ''.join([
        self.credentials[0], ':', self.challenge['realm'], ':',
        self.credentials[1]
    ])
    self.challenge['nc'] = 1
  ...
  def request(self, method, request_uri, headers, content, cnonce=None):
    ...
    request_digest = '"%s"' % KD(
        H(self.A1), '%s:%s:%s:%s:%s' %
        (self.challenge['nonce'], '%08x' % self.challenge['nc'],
         self.challenge['cnonce'], self.challenge['qop'], H(A2)))
    headers['authorization'] = (
        'Digest username="%s", realm="%s", nonce="%s", '
        'uri="%s", algorithm=%s, response=%s, qop=%s, '
        'nc=%08x, cnonce="%s"') % (self.credentials[0], self.challenge['realm'],
                                   self.challenge['nonce'], request_uri,
                                   self.challenge['algorithm'], request_digest,
                                   self.challenge['qop'], self.challenge['nc'],
...
class HmacDigestAuthentication(Authentication):
  ...
  def __init__(self, credentials, host, request_uri, headers, response, content,
    ...
    self.key = ''.join([
        self.credentials[0], ':',
        self.pwhashmod.new(''.join(
            [self.credentials[1], self.challenge['salt']])).hexdigest().lower(),
  ...
  def request(self, method, request_uri, headers, content):
    ...
    ) % (self.credentials[0], self.challenge['realm'], self.challenge['snonce'],
         cnonce, request_uri, created, request_digest, keylist)
...
class WsseAuthentication(Authentication):
  ...
  def request(self, method, request_uri, headers, content):
    ...
    headers['X-WSSE'] = ('UsernameToken Username="%s", PasswordDigest="%s", '
                         'Nonce="%s", Created="%s"') % (self.credentials[0],
                                                        password_digest, cnonce,
                                                        iso_now)
...
class ProxyInfo(object):
    ...
    """
        Args:
          ...
          proxy_rdns: If True (default), DNS queries will not be performed
            locally, and instead, handed to the proxy to resolve.  This is
            useful if the network does not allow resolution of non-local names.
            In httplib2 0.9 and earlier, this defaulted to False.
  ...
  def __repr__(self):
    return ('<ProxyInfo type={p.proxy_type} '
    ...
    for res in socket.getaddrinfo(host, port, 0, socket.SOCK_STREAM):
      ...
      try:
        if use_proxy:
          ...
          self.sock.setproxy(proxy_type, proxy_host, proxy_port, proxy_rdns,
        ...
        if self.debuglevel > 0:
          ...
          if use_proxy:
            print('proxy: %s' % str((proxy_host, proxy_port, proxy_rdns,
                                     proxy_user, proxy_pass, proxy_headers)))
        if self.sock:
          self.sock.close()
        self.sock = None
...
class HTTPSConnectionWithTimeout(httplib.HTTPSConnection):
  ...
  def _GetValidHostsForCert(self, cert):
    ...
    if 'subjectAltName' in cert:
      ...
      return [
          x[0][1] for x in cert['subject'] if x[0][0].lower() == 'commonname'
      ]
    ...
    for family, socktype, proto, canonname, sockaddr in address_info:
      try:
        if use_proxy:
          ...
          sock.setproxy(proxy_type, proxy_host, proxy_port, proxy_rdns,
                        proxy_user, proxy_pass, proxy_headers)
        else:
        ...
        self.sock = _ssl_wrap_socket(sock, self.key_file, self.cert_file,
                                     self.disable_ssl_certificate_validation,
                                     self.ca_certs, self.ssl_version, self.host)
        if self.debuglevel > 0:
          print('connect: (%s, %s)' % (self.host, self.port))
          if use_proxy:
            print('proxy: %s' % str((proxy_host, proxy_port, proxy_rdns,
        ...
        if not self.disable_ssl_certificate_validation:
          ...
          if not self._ValidateCertificateHostname(cert, hostname):
            raise CertificateHostnameMismatch(
              ...
              CertificateHostnameMismatch) as e:
        if sock:
          sock.close()
        if self.sock:
          self.sock.close()
    ...
    for cred in self.credentials.iter(host):
      for scheme in AUTH_SCHEME_ORDER:
        if scheme in challenges:
          yield AUTH_SCHEME_CLASSES[scheme](cred, host, request_uri, headers,
    ...
    if (self.follow_all_redirects or (method in ['GET', 'HEAD']) or
      ...
      if self.follow_redirects and response.status in [300, 301, 302, 303, 307]:
        ...
        if redirections:
          ...
          raise RedirectLimit(
              'Redirected more times than rediection_limit allows.', response,
              content)
...
# including all socket.* and httplib.* exceptions.
  ...
  def request(self,
    ...
    try:
      ...
      if cached_value and method in ['GET', 'HEAD'
        ...
        if '-x-permanent-redirect-url' in info:
          ...
          (response, new_content) = self._request(
              conn, authority, uri, request_uri, method, body, headers,
        ...
        if 'only-if-cached' in cc:
          ...
          (response,
           content) = self._request(conn, authority, uri, request_uri, method,
  ...
  def _get_proxy_info(self, scheme, authority):
    ...
    if (hasattr(proxy_info, 'applies_to') and
        not proxy_info.applies_to(hostname)):
      proxy_info = None
    return proxy_info
