...
class ParserTest(unittest.TestCase):
  ...
  def testFromStd66(self):
    self.assertEqual(('http', 'example.com', '', None, None),
                     httplib2.parse_uri('http://example.com'))
    self.assertEqual(('https', 'example.com', '', None, None),
                     httplib2.parse_uri('https://example.com'))
    self.assertEqual(('https', 'example.com:8080', '', None, None),
                     httplib2.parse_uri('https://example.com:8080'))
    self.assertEqual(('http', 'example.com', '/', None, None),
                     httplib2.parse_uri('http://example.com/'))
    self.assertEqual(('http', 'example.com', '/path', None, None),
                     httplib2.parse_uri('http://example.com/path'))
    self.assertEqual(('http', 'example.com', '/path', 'a=1&b=2', None),
                     httplib2.parse_uri('http://example.com/path?a=1&b=2'))
    self.assertEqual(('http', 'example.com', '/path', 'a=1&b=2', 'fred'),
...
class UrlNormTest(unittest.TestCase):
  ...
  def test(self):
    ...
    self.assertEqual('http://example.org/',
                     httplib2.urlnorm('http://EXAMple.org')[-1])
    self.assertEqual('http://example.org/?=b',
                     httplib2.urlnorm('http://EXAMple.org?=b')[-1])
    self.assertEqual('http://example.org/mypath?a=b',
                     httplib2.urlnorm('http://EXAMple.org/mypath?a=b')[-1])
    self.assertEqual('http://localhost:80/',
...
class UrlSafenameTest(unittest.TestCase):
  ...
  def test(self):
    ...
    self.assertEqual('example.org,fred,a=b,58489f63a7a83c3b7794a6a398ee8b1f',
                     httplib2.safename('http://example.org/fred/?a=b'))
    self.assertEqual('example.org,fred,a=b,8c5946d56fec453071f43329ff0be46b',
                     httplib2.safename('http://example.org/fred?/a=b'))
    self.assertEqual(
...
class HttpTest(unittest.TestCase):
  ...
  def testGetConnectionRefused(self):
    ...
    try:
      ...
      self.fail(
          'An socket.error exception must be thrown on Connection Refused.')
    except socket.error:
      pass
  ...
  def testGet301ViaHttps(self):
    ...
    (response, content) = self.http.request('https://code.google.com/apis/',
                                            'GET')
  ...
  def testGetViaHttps(self):
    ...
    (response, content) = self.http.request('https://www.google.com/adsense/',
                                            'GET')
    self.assertEqual(200, response.status)
  ...
  def testSslCertValidationWithoutSslModuleFails(self):
    if sys.version_info < (2, 6):
      ...
      self.assertRaises(httplib2.CertificateValidationUnsupported, http.request,
  ...
  def testGetViaHttpsKeyCert(self):
    ...
    try:
      ...
      pass
    self.assertEqual(http.connections['https:bitworking.org'].key_file,
                     'akeyfile')
    self.assertEqual(http.connections['https:bitworking.org'].cert_file,
                     'acertfile')

    try:
      (response, content) = http.request('https://notthere.bitworking.org',
                                         'GET')
    except:
      pass
    self.assertEqual(http.connections['https:notthere.bitworking.org'].key_file,
                     None)
    self.assertEqual(
  ...
  def test303ForDifferentMethods(self):
    ...
    for (method, method_on_303) in [('PUT', 'GET'), ('DELETE', 'GET'),
                                    ('POST', 'GET'), ('GET', 'GET'), ('HEAD',
                                                                      'GET')]:
      (response, content) = self.http.request(uri, method, body=' ')
  ...
  def testGet304(self):
    ...
    cache_file_name = os.path.join(cacheDirName,
                                   httplib2.safename(httplib2.urlnorm(uri)[-1]))
...
#        self.assertEqual(response.fromcache, True)
  ...
  def testGet304LastModified(self):
    ...
    uri = urlparse.urljoin(base,
                           '304/last-modified-only/last-modified-only.txt')
    (response, content) = self.http.request(uri, 'GET')
  ...
  def testGetDuplicateHeaders(self):
    ...
    self.assertEqual(response['link'].split(',')[0],
...
class HttpPrivateTest(unittest.TestCase):
  ...
  def testExpirationModelDateOnly(self):
    ...
    response_headers = {
        'date':
            time.strftime('%a, %d %b %Y %H:%M:%S GMT', time.gmtime(now + 3)),
    }
    request_headers = {}
    self.assertEqual(
        'STALE', httplib2._entry_disposition(response_headers, request_headers))
  ...
  def testParseWWWAuthenticateMoreQuoteCombos(self):
    res = httplib2._parse_www_authenticate({
        'www-authenticate':
            'Digest realm="myrealm", nonce="Ygk86AsKBAA=3516200d37f9a3230352fde99977bd6d472d4306", algorithm=MD5, qop="auth", stale=true'
    })
    digest = res['digest']
  ...
  def testDigestObject(self):
    ...
    d = httplib2.DigestAuthentication(credentials, host, request_uri, headers,
  ...
  def testWsseAlgorithm(self):
    digest = httplib2._wsse_username_token('d36e316282959a9ed4c89851497a717f',
