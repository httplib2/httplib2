...
class HmacDigestAuthentication(Authentication):
  ...
  def request(self, method, request_uri, headers, content):
    ...
    headers[
        'authorization'] = ('HMACDigest username="%s", realm="%s", snonce="%s",'
        ...
        if self.debuglevel > 0:
          ...
          if use_proxy:
            print('proxy: {0} ************'.format(
                str((proxy_host, proxy_port, proxy_rdns, proxy_user, proxy_pass,
                     proxy_headers))))
  ...
  def connect(self):
    ...
    for family, socktype, proto, canonname, sockaddr in address_info:
      try:
        ...
        if not hasattr(
            self._context,
            'check_hostname') and not self.disable_ssl_certificate_validation:
          try:
