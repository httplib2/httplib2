...
def test_user_agent():
  ...
  with tests.server_reflect() as uri:
    ...
    assert response.status == 200
    reflected = tests.HttpRequest.from_bytes(content)
    assert reflected.headers.get('user-agent',
                                 '').startswith('Python-httplib2/')
...
def test_get_300_without_location():
  ...
  with tests.server_const_http(
      status='300 Multiple Choices', body=b'redirect body') as uri:
    response, content = http.request(uri, 'GET')
  assert response.status == 300
  assert response.previous is None
...
def test_etag_used():
  ...
  with tests.server_request(handler, request_count=2) as uri:
    ...
    cache_file_name = os.path.join(cache_path,
