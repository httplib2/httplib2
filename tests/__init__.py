...
def parse_http_message(kind, buf):
    ...
    if kind is HttpRequest:
        ...
        msg.method, msg.uri, msg.proto = start_line.rstrip().decode().split(
            ' ', 2)
        ...
        msg.proto, msg.status, msg.reason = start_line.rstrip().decode().split(
            ' ', 2)
        msg.status = int(msg.status)
        assert msg.proto.startswith('HTTP/'), repr(start_line)
    else:
        raise Exception(
            'Use HttpRequest or HttpResponse .from_{bytes,buffered}')
...
def http_response_bytes(proto='HTTP/1.1',
                        status='200 OK',
                        headers=None,
                        body=b'',
    ...
    if add_etag:
        headers.setdefault('etag', '"{0}"'.format(
...
def make_http_reflect(**kwargs):
    assert 'body' not in kwargs, ('make_http_reflect will overwrite response '
                                  'body')
...
def server_const_http(**kwargs):
    response_kwargs = {
        k: kwargs.pop(k) for k in dict(kwargs) if k in _http_kwargs
    }
...
def http_reflect_with_auth(allow_scheme,
                           allow_credentials,
                           out_renew_nonce=None,
                           out_requests=None):
    ...
    def renew_nonce():
        if gnextnonce[0]:
            assert False, ('previous nextnonce was not used, probably bug in '
                           'test code')
    ...
    def deny(**kwargs):
        ...
        if allow_scheme == 'basic':
            ...
            authenticate = ('digest realm="{realm}", qop="auth"' +
                            ', nonce="{nonce}", opaque="{opaque}"' +
                            (', stale=true' if nonce_stale else '')).format(
                                realm=realm,
                                nonce=gserver_nonce[0],
    ...
    def http_reflect_with_auth_handler(request):
        ...
        if scheme == 'basic':
            ...
            ha2 = hasher(':'.join((request.method,
                                   request.uri)).encode()).hexdigest()
            ...
            if client_opaque != server_opaque:
                return deny(
                    body='auth-info opaque mismatch expected={} actual={}'
                    .format(server_opaque, client_opaque).encode())
            for allow_username, allow_password in allow_credentials:
                ha1 = hasher(':'.join((allow_username, realm,
                                       allow_password)).encode()).hexdigest()
